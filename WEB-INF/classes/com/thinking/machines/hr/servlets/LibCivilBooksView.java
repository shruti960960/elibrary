...
import java.io.*;
import java.util.*;
public class LibCivilBooksView extends HttpServlet
{
...
String publisher;
int sno=0;
CivilBooksDAO civilBooksDAO=new CivilBooksDAO();
List<CivilBooksDTO>civilBooks;
civilBooks=civilBooksDAO.getAll();
for(int x=0;x<civilBooks.size();x++)
{
sno++;
civilBooksDTO=civilBooks.get(x);
book_id=civilBooksDTO.getBookId();
book_name=civilBooksDTO.getBookName();
author_name=civilBooksDTO.getAuthorName();
publisher=civilBooksDTO.getPublisher();
pw.println("<tr>");
