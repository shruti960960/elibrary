...
import java.io.*;
import java.util.*;
public class LibECBooksView extends HttpServlet
{
...
String publisher;
int sno=0;
ECBooksDAO ECBooksDAO=new ECBooksDAO();
List<ECBooksDTO>ECbooks;
ECbooks=ECBooksDAO.getAll();
for(int x=0;x<ECbooks.size();x++)
{
sno++;
ECBooksDTO=ECbooks.get(x);
book_id=ECBooksDTO.getBookId();
book_name=ECBooksDTO.getBookName();
author_name=ECBooksDTO.getAuthorName();
publisher=ECBooksDTO.getPublisher();
pw.println("<tr>");
...
pw.println("<td style='text-align:center'><a href='/elibrary1/editECBooks?book_id="+book_id+"'>Edit</a></td>");
pw.println("<td style='text-align:center'><a href='/elibrary1/deleteECBooks?book_id="+book_id+"'>Delete</a></td>");
pw.println("</tr>");
}
