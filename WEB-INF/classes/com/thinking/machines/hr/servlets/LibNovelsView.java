...
import java.io.*;
import java.util.*;
public class LibNovelsView extends HttpServlet
{
...
String publisher;
int sno=0;
NovelDAO novelsDAO=new NovelDAO();
List<NovelDTO>novels;
novels=novelsDAO.getAll();
for(int x=0;x<novels.size();x++)
{
sno++;
novelsDTO=novels.get(x);
book_id=novelsDTO.getBookId();
book_name=novelsDTO.getBookName();
author_name=novelsDTO.getAuthorName();
publisher=novelsDTO.getPublisher();
pw.println("<tr>");
